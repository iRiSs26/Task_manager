  ...
  return (
    <div className="p-4 border rounded bg-white shadow-md">
      ...
      <div className="space-y-4">
        {tasks.map(task => (
          <div key={task.id} className="p-4 border rounded bg-gray-100 relative flex flex-col">
            <h3 className="text-lg font-medium mb-2">{task.title}</h3>
            <p className="text-gray-700 mb-2">{task.description}</p>
            <div className="text-gray-600 mb-2">
              <span className={`font-semibold ${task.priority === 'High' ? 'text-red-600' : task.priority === 'Medium' ? 'text-yellow-600' : 'text-green-600'}`}>{task.priority}</span>
            </div>
            <div className="text-gray-600 mb-2">
